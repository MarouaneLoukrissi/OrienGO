...
export class StudentDashboardComponent implements OnInit {
  // Current user ID (hardcoded for now as requested)
  currentUserId = 1;
  ...
  private loadStudentProfiles(): void {
    ...
    this.testResultService.getProfilesByStudentId(this.currentUserId).subscribe({
      next: (response) => {
      ...
      error: (error) => {
        ...
        this.handleError('Failed to load student profiles. Please try again.');
        this.isLoading = false;
      }
    });
  }
