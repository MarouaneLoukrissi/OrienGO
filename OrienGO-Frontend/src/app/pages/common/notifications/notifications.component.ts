...
export class NotificationsComponent {
  ...
  get availableFilters() {
    const baseFilters = [
      { key: 'all', label: 'NOTIFICATIONS.FILTERS.ALL', count: this.filteredNotifications.length },
      { key: 'unread', label: 'NOTIFICATIONS.FILTERS.UNREAD', count: this.unreadCount }
    ];

    if (this.currentUserRole === 'student') {
      return [
        ...baseFilters,
        { key: 'job', label: 'NOTIFICATIONS.FILTERS.JOBS', count: this.getFilterCount('job') },
        { key: 'test', label: 'NOTIFICATIONS.FILTERS.TESTS', count: this.getFilterCount('test') },
        { key: 'coaching', label: 'NOTIFICATIONS.FILTERS.COACHING', count: this.getFilterCount('coaching') }
      ];
    } else {
      return [
        ...baseFilters,
